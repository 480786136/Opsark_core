...
fn endpoint_url(endpoint: &str) -> Result<Url, String> {
    ...
    {
        return Err("知识地址须为 HTTPS /api/v1；仅本机调试允许 HTTP，不允许用户名、查询参数或片段".into());
    }
    Ok(url)
}
...
pub(crate) async fn knowledge_request(
    ...
    let key = crate::credential::load_credential("knowledge".into(), credential_id)?
        .filter(|v| !v.is_empty()).ok_or("知识 API Key 尚未保存到系统钥匙串")?;
    let client = reqwest::Client::builder().redirect(Policy::none()).no_proxy()
        .connect_timeout(Duration::from_secs(8)).timeout(Duration::from_secs(25))
        .build().map_err(|_| "无法创建知识连接")?;
    let url = format!("{}/{path}", base.as_str().trim_end_matches('/'));
    let mut request = if operation == "upload" {
        let body = body.ok_or("缺少上传正文")?;
        if body.len() > 256 * 1024 { return Err("上传正文超过 256 KiB".into()); }
        ...
        client.post(&url).header("Content-Type", "application/json").header("Idempotency-Key", idem).body(body)
    } else { client.get(&url) };
    request = request.bearer_auth(key).header("Accept", "application/json");
    // Do not reuse the model request logger: knowledge payloads and credentials stay out of logs.
    let mut response = request.send().await.map_err(|_| "知识服务连接失败或超时，请检查地址后手动重试")?;
    let status = response.status().as_u16();
    let retry_after_seconds = response.headers().get("retry-after").and_then(|v| v.to_str().ok()).and_then(|v| v.parse().ok());
    let mut bytes = Vec::new();
    while let Some(part) = response.chunk().await.map_err(|_| "知识响应读取失败，请手动重试")? {
        if bytes.len() + part.len() > 2 * 1024 * 1024 { return Err("知识响应过大".into()); }
        bytes.extend_from_slice(&part);
    }
    let parsed: Value = serde_json::from_slice(&bytes).unwrap_or(Value::Null);
    let data = if (200..300).contains(&status) {
        if parsed.is_null() { return Err("知识服务未返回有效 JSON".into()); }
        parsed
    } else {
        let code = parsed.pointer("/error/code").and_then(Value::as_str).unwrap_or("HTTP_ERROR");
        let safe_code: String = code.chars().filter(|c| c.is_ascii_uppercase() || *c == '_').take(64).collect();
        json!({"error": {"code": safe_code}})
    };
    Ok(KnowledgeResponse { status, data, retry_after_seconds })
}
...
mod tests {
    ...
    fn validates_destination_and_paths() {
        ...
        for invalid in ["http://remote.example/api/v1", "https://user:key@host/api/v1", "https://host/internal/v1", "https://host/api/v1?key=x", "file:///api/v1"] {
        ...
        assert_eq!(operation_path("status", Some("rec-1")).unwrap(), "records/rec-1");
    }
}
