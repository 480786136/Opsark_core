...
2026-09-16 起，桌面端的“操作日志”和“开发者日志”会通过受限的 `query_task_logs` 命令回读对应 JSONL，按服务器、任务、类别/操作/传输事件、级别、日期和关键词过滤，并使用不透明游标分页。Pinia/`localStorage` 只保留实时窗口和纯浏览器运行时的降级快照，不再决定桌面端可查历史的上限。第一页查询会固定各分卷的文件长度，后续页在同一份有界快照上继续读取，因此分页期间的新日志不会改变总数或造成重复、漏项；快照采用 15 分钟滑动有效期，过期后在界面刷新即可重新查询。读取端不接受任意文件路径，只能读取应用数据目录内的白名单日志流与数字分卷；单次查询枚举的任务目录和分卷文件分别以 32,768 个为硬上限，并在第一个超限目标出现时立即拒绝。单行和单页上限均为 8 MiB，查询候选记录总内存上限为 16 MiB，损坏行和超限行会被统计并在界面告警。查询只在写锁下截取文件长度，随后在后台阻塞任务中扫描，不会读到追加中的半行，也不会在整个历史查询期间阻塞新日志写入。

`model-calls` 仍作为更敏感的底层传输记录保留在磁盘，不会混入默认的“脱敏业务诊断”列表。用户切换到“模型传输元数据”后才会查询该流；Rust 只向 Renderer 返回调用 ID、阶段、时间、HTTP 状态、Token/缓存计数和上下文体积等白名单元数据，`request` / `response` / `responseText` / `error` 正文、URL 和文件路径不跨 IPC 返回，搜索也只匹配安全投影。现有 JSONL 查询是 SQLite/WAL 审计仓储完成前的过渡闭环；每个新筛选条件仍需扫描历史，分卷也不会自动删除，保留期、总配额、索引化查询和合规销毁策略仍需按商业化路线实施。
