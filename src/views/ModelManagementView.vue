...
async function save() {
  ...
  try {
    ...
    if (draft.value.source !== "official" && ![draft.value.name, draft.value.model, draft.value.endpoint].every(value => value.trim())) {
    ...
    if (model.source === "official") {
      ...
      Object.assign(current, { timeoutSeconds, requestParameters: model.requestParameters });
            ...
            <form class="drawer-form" @submit.prevent="save">
              <fieldset :disabled="busy" class="drawer-body">
                ...
                <div class="connection-fields">
                  <label v-if="draft.source !== 'official'" class="full"><span>{{ t('settings.configName') }}</span><input v-model="draft.name" required/></label>
