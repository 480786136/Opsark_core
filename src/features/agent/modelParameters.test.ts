...
it("keeps defaults unchanged and preserves zero overrides and evidence", () => {
  ...
  const model = { requestParameters: { temperature: 0 }, timeoutSeconds: 240, model: "example", endpoint: "example" } as ModelProfile;
  expect(createRuntimeModel(model, "key", "{}")?.requestParameters).toEqual({ temperature: 0 });
  expect(createRuntimeModel(model, "key", "{}")?.timeoutSeconds).toBe(240);
});
