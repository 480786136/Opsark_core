...
const agentTasks = computed(() => ops.tasks.filter((task) => (
  (task.executionTargetServerId ?? task.serverId) === props.serverId && Boolean(task.agentSessionId)
)));
    ...
    <header class="terminal-tabs-bar">
      <div class="terminal-tab-list" role="tablist">
        ...
        >
          ...
          <button type="button" class="terminal-tab-close" title="隐藏 Agent 终端" @click.stop="activateShell(activeSessionId)"><X :size="12" /></button>
        </div>
      </div>
