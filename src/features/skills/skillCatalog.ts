...
1. 先确认源服务器、源文件绝对路径、目标服务器和包含文件名的目标绝对路径。仅当任务必需信息确实缺失时，单独调用 opsark-tool user.request_input；每个字段必须说明参数及用途。
2. 传输前在源服务器执行只读检查，确认源对象是存在、可读的普通文件，并获取真实绝对路径、字节数和 SHA-256。不得把目录、空匹配、同名文件或仅有路径文本当作已确认的源文件。
3. 在源服务器只读检查到目标 SSH 端口的网络连通性，并区分“网络不可达”和“SSH 认证不可用”。网络可达时，再用不提示密码的非交互 SSH 探测确认源服务器是否已有可用的目标端认证。不得把 Opsark 保存的密码写入命令、日志、文件或源服务器，也不得用 sshpass 等方式暴露密码。
4. 源服务器能连接目标 SSH 且已有源端可用认证时，优先在源服务器以前台 scp 直接传输。命令必须保留真实退出码并由执行器跟踪到结束，不得放入后台或掩盖失败。目标先写入同目录临时文件；确认传输成功及完整性后再原子替换最终路径。
5. 只有源服务器到目标服务器的网络确实不通，并且 Opsark 已有证据证明能分别 SSH 连接源、目标两台服务器时，才回退调用 opsark-tool files.transfer_between_servers，由桌面后端使用受管凭据流式中转。目标为地址且连接资料证据不足时，先单独调用 opsark-tool server.resolve_connection；模型不得读取或输出密码。任一端未纳管、凭据不可用或 Opsark 无法连接时，报告阻断及缺失条件。
6. 若服务器间网络可达但源服务器缺少目标端 SSH 认证，不得谎报网络不通或直接改用中转工具；应报告认证前提缺失，并请求用户提供合规的源端认证方案或明确调整传输路径。
7. overwrite 默认为 false。只有用户明确允许覆盖，或真实证据确认目标文件可替换时，才能替换最终路径。不得静默覆盖未知同名文件；临时文件在失败、终止或校验不一致时应安全清理。
8. 无论使用 scp 还是中转工具，完成后都必须在目标服务器独立读取最终文件的字节数和 SHA-256，并与源文件证据逐项一致。中转工具返回的 transferredBytes、targetPath 和 sha256 也必须与源证据及目标复核一致。传输中断、退出码非零、用户终止或校验失败都不得报告成功。
每轮只规划当前证据允许的最少步骤；已获得的路径、字节数、哈希、网络状态、认证状态和目标资料必须复用，不得无证据重复发现或重复传输。`,
